...
class PipelineExecutor:
    ...
    async def execute(
        self, query: str, top_k: int = 5, generate_response: bool = True
    ) -> PipelineResult:
        """
        ...
        try:
            ...
            # Step 2: Retrieval
            search_results = self.hybrid_search.search(preprocessed.anonymized_text, top_k=top_k)

            # Step 3: Routing decision
            routing_decision = self.router.route(preprocessed.anonymized_text, search_results)
            ...
            if generate_response and routing_decision.route == "auto_response":
                ...
                response = self.llm_service.generate_response(
                    query=preprocessed.anonymized_text, context=context, stream=False
                )
            ...
            result = PipelineResult(
                ...
                metadata={"top_k": top_k, "retrieval_count": len(search_results)},
            )
